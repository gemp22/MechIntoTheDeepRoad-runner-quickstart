...
import static org.firstinspires.ftc.teamcode.RobotPosition.subtractAngles;
...
import org.firstinspires.ftc.teamcode.MyMath;
...
public class Movement {
    ...
    public static double movement_y_min = 0.1423 +0.025; // .1382 before retuning at 1/25 qual
    public static double movement_x_min = 0.2363 +0.025; // was .2043 before 1/25 retuning
    public static double movement_turn_min = 0.1881 +0.025; //was 0.1881 0.1642 before 1/25 retuning
    ...
    public static movementResult gunToPosition(double targetX, double targetY, double point_angle,
        ...
        if (stop) {
            movement_x_power *= relative_abs_x / 11.8;
            movement_y_power *= relative_abs_y / 11.8;
        }
...
//        movement_x = Range.clip(movement_y_power, -movement_speed, movement_speed);
        ...
        movement_x *= Range.clip((relative_abs_x / 0.4), 0, 1);
        movement_y *= Range.clip((relative_abs_y / 0.4), 0, 1);
    ...
    public static boolean followCurve(ArrayList<CurvePoint> allPoints, double followAngle, double tolerance) {
        ...
        gunToPosition(followMe.x, followMe.y, followAngle,
                ...
                followMe.slowDownTurnRadians, 0, currFollowIndex >= allPoints.size()-1);
        ...
        return clipedDistToFinalEnd < tolerance;//if we are less than 10 cm to the target, return true
    }
    ...
    public static pointWithIndex clipToPath(ArrayList<CurvePoint> pathPoints, double xPos, double yPos) {
        ...
        PointDouble clippedToLine = new PointDouble();//this is the clipped point where it is closest
        ...
        for (int i = 0; i < pathPoints.size() - 1; i++) {
            ...
            PointDouble currClippedToLine = clipToLine(firstPoint.x, firstPoint.y,
    ...
    public static CurvePoint getFollowPointPath(ArrayList<CurvePoint> pathPoints,
        ...
        followMe.setPoint(new PointDouble(clippedToLine.x, clippedToLine.y));
        ...
        for (int i = 0; i < pathPoints.size() - 1; i++) {
            ...
            ArrayList<PointDouble> intersections =
            ...
            for (int p = 0; p < intersections.size(); p++) {
                ...
                PointDouble thisIntersection = intersections.get(p);
...
//    }
    ...
    public static PointDouble clipToLine(double lineX1, double lineY1, double lineX2, double lineY2,
        ...
        return new PointDouble(xClipedToLine, yClipedToLine);
    }

    //extends a point on a line in the direction of the two points defining the line (order matters)
    //this also clips it
    public static PointDouble extendPointOnLine(double lineX1, double lineY1, double lineX2, double lineY2,
                                                double pointX, double pointY, double extendDist) {
        PointDouble cliped = clipToLine(lineX1, lineY1, lineX2, lineY2, pointX, pointY);
        ...
        return new PointDouble(xTarget, yTarget);
    }

    public static myPoint pointAlongLine(double lineX1, double lineY1, double lineX2, double lineY2,
                                         double robotX, double robotY,
                                         double followDistance) {
        PointDouble clipedToLine = clipToLine(lineX1, lineY1, lineX2, lineY2, robotX, robotY);
