...
public class Superstructure3Motor {
    ...
    boolean isPivotUpForCollection = false;
    ...
    public enum SuperstructureStates {
        ...
        GOTO_RESTING_WORLDS,
        RESTING_WORLDS,
        RESTING_CHAMBER_COLLECTION,

        PICKUP,
        DELIVERY_LEVEL_1,
        DELIVERY_LEVEL_2,
        DELIVERY_SAMPLE_DROP,
        DELIVERY_LEVEL_1_AUTO,
        DELIVERY_LEVEL_2_AUTO,
...
//                pivotStateStartAngle = armPivot.getArmAngle();
                ...
                armPivot.intakeTiltTwo.setPosition(Constants3Motor.TILT_SERVO_PARALLEL_WITH_PIVOT);
            ...
            if(isLiftOrPivotSmall){
                ...
                if(SystemClock.uptimeMillis() - taskStartTime > 750){ //wait for jaw and tilt
                    ...
                    if (lift.getLiftExtension()<3 && armPivot.getArmAngle() > -3 && (SystemClock.uptimeMillis() - taskStartTime) > 1500 && !collectionMode) { //gives time for twist before Pivot goes down
                ...
                if (lift.getLiftExtension() < 13 && armPivot.getArmAngle() > -5 && !collectionMode) {
            ...
            if (ButtonPress.isGamepad2_left_stick_button_pressed() && lift.getLiftExtension() > 0.9) {
                if (!collectionMode) {
            ...
            Log.i("pivot angle: ", String.valueOf(armPivot.getArmAngle()));
            Log.i("pivot power left: ", String.valueOf(armPivot.armPivotLeft.getPower()));
            Log.i("pivot power right: ", String.valueOf(armPivot.armPivotRight.getPower()));

            if (collectionMode) {
                armPivot.setIntakeTiltAngle(armPivot.intakeTiltNoArmPower(lift.getLiftExtension()));
            }

            if (lift.getLiftExtension() < 0.9 && !outTakeMode && armPivot.getArmAngle()<10 && collectionMode) {
                collectionMode = false;
                armPivot.setIntakeTiltAngle(Constants3Motor.TILT_SERVO_HOME_STATE_ANGLE);
            }
        ...
        if (currentState == SuperstructureStates.GOTO_RESTING_WORLDS.ordinal()) {
            if (stateFinished) {
                restingStateStartingAngle = armPivot.getArmAngle();
                // init vars as needed
                setToHomeResting = false;
...
//                armPivot.intakeTiltTwo.setPosition(Constants3Motor.TILT_SERVO_90_DEGREES_UP);
            ...
            if (lift.getLiftExtension()<1 && armPivot.getArmAngle()<0){
                nextState(SuperstructureStates.RESTING_WORLDS.ordinal());
            }
            lift.setSetPoint(liftWantedHeight);
            lift.updateLiftPosition();
        }

        if (currentState == SuperstructureStates.RESTING_WORLDS.ordinal()) {
            if (stateFinished) {
                restingStateStartingAngle = armPivot.getArmAngle();
                // init vars as needed
                setToHomeResting = false;
        ...
        if(currentState == SuperstructureStates.RESTING_CHAMBER_COLLECTION.ordinal())
        {
            if (ButtonPress.isGamepad2_left_stick_button_pressed() && lift.getLiftExtension() > 0.9) {
                if (!collectionMode) {
                    armPivot.intakeJawServo.setPosition(Constants3Motor.JAW_SERVO_INTAKE_POSITION);
...
//                }
            ...
            //armPivot.setIntakeTiltAngle(0);
//            armPivot.update2(94.5, 1, 10, 0.75, telemetry);
            //armPivot.setArmPivotSetPointTicks(armPivot.armPivot.getCurrentPosition());
            if (!armPivot.getPivotLimitState() && armPivot.getArmAngle() > Constants3Motor.ARM_PIVOT_DELIVERY_ANGLE - 2) {
                    armPivot.setArmPivotPower(0.25);
            } else {

                armPivot.update2(Constants3Motor.ARM_PIVOT_DELIVERY_ANGLE, 0.65, 30, 0.20, telemetry);
            }
...
//            }
        ...
        if (currentState == SuperstructureStates.DELIVERY_LEVEL_1_AUTO.ordinal()) {
            if (stateFinished) {
                deliveryTIlt = false;
                deliveryTwist = false;
                liftWantedHeight = 9;
...
//            armPivot.update2(94.5, 1, 10, 0.75, telemetry);
        ...
        if (currentState == SuperstructureStates.DELIVERY_LEVEL_2.ordinal()) {
            ...
            if (armPivot.getArmAngle() > 25) {
                Log.i("lift is extending", "arm angle:" + armPivot.getArmAngle() + " lift extension: " + lift.getLiftExtension() + " lift power R/L: " + lift.liftRight.getPower() + "/" + lift.liftRight.getPower() );
...
//                }
            ...
            if (!armPivot.getPivotLimitState() && armPivot.getArmAngle() > Constants3Motor.ARM_PIVOT_DELIVERY_ANGLE - 2) {
                ...
                armPivot.update2(Constants3Motor.ARM_PIVOT_DELIVERY_ANGLE, 0.70, 30, 0.20, telemetry);
            }
                //armPivot.setIntakeTiltAngle(0);
                //armPivot.update2(94.5, 0.99, 10, 0.75, telemetry);
                //armPivot.setArmPivotSetPointTicks(armPivot.armPivot.getCurrentPosition());

        }

        if (currentState == SuperstructureStates.DELIVERY_LEVEL_2_AUTO.ordinal()) {
            if (stateFinished) {
                liftWantedHeight = 24;
                initializeStateVariables();
            }

            if (armPivot.getArmAngle() > 70) {
                Log.i("lift is extending", "arm angle:" + armPivot.getArmAngle() + " lift extension: " + lift.getLiftExtension() + " lift power R/L: " + lift.liftRight.getPower() + "/" + lift.liftRight.getPower() );
        ...
        if (currentState == SuperstructureStates.DELIVERY_SAMPLE_DROP.ordinal()) {
            if (stateFinished) {
                ...
                armPivot.intakeTiltTwo.setPosition(Constants3Motor.TILT_SERVO_BASKET_DELIVERY);
                restingStateStartingAngle = armPivot.getArmAngle();
                initializeStateVariables();
            }
        ...
        if (currentState == SuperstructureStates.HANG_BAR_1_PREP.ordinal()) {
            ...
            armPivot.twist.setPosition(Constants3Motor.TWIST_SERVO_HORIZONTAL_POSITION);
            armPivot.setIntakeTiltAngle(90);
            //armPivot.update(53, 0.8, 10, 0.45, telemetry); // 1 pivot motor
            armPivot.update(51, 0.75, 30, 0.2, telemetry); // 2 pivot motor
        }
        ...
        if (currentState == SuperstructureStates.HANG_BAR_1.ordinal()) {
            ...
            System.out.println("HANG DEBUG BAR 1 Arm power left: " + armPivot.armPivotLeft.getPower());
...
//                lift.updateLiftPosition();
        ...
        if (currentState == SuperstructureStates.HANG_BAR_2_PREP.ordinal()) {
            if (stateFinished) {
                ...
                hangPivotAngleToBarTwoPrep = 19.0;
            ...
            System.out.println("HANG DEBUG BAR 2 PREP Arm power left: " + armPivot.armPivotLeft.getPower());
            System.out.println("HANG DEBUG BAR 2 PREP Arm power right: " + armPivot.armPivotRight.getPower());
            ...
            if (liftIsReadyForBar2Pivot && !liftIsReadyToAttachToBar2) {
                ...
                if (armPivot.getArmAngle() > 13 && okToExtendLiftToBar2) {
                    armPivot.setIntakeTiltAngle(40);
        ...
        if (currentState == SuperstructureStates.HANG_BAR_TWO_PREP_RESTING.ordinal()) {
            ...
            System.out.println("HANG DEBUG BAR 2 RESTING Arm power left: " + armPivot.armPivotLeft.getPower());
        ...
        if (currentState == SuperstructureStates.HANG_BAR_2.ordinal()) {
            ...
            System.out.println("HANG DEBUG BAR2  Arm power left: " + armPivot.armPivotLeft.getPower());
        ...
        if (currentState == SuperstructureStates.HANG_BAR_2_DONE_DONE_DONE.ordinal()) {
            ...
            System.out.println("HANG DEBUG BAR 2 DONE Arm power Left: " + armPivot.armPivotLeft.getPower());
...
//                liftExtenedToTipBotForward = true;
        ...
        if (currentState == SuperstructureStates.COLLECT_SPECIMEN_PREP.ordinal()) {
            if (stateFinished) {
                targetPivotAngle = 11;   // adjust this for optimal specimen on the wall height
                liftWantedHeight = 4;
                armPivot.setIntakeTiltAngle(0);
                initializeStateVariables();
            }
            ...
            if (armPivot.getArmAngle() > 3) {
                ...
                armPivot.intakeTilt.setPosition(Constants3Motor.TILT_SERVO_PARALLEL_WITH_FLOOR);
                armPivot.intakeTiltTwo.setPosition(Constants3Motor.TILT_SERVO_PARALLEL_WITH_FLOOR);
                armPivot.intakeJawServo.setPosition(Constants3Motor.JAW_SERVO_WALL_COLLECTION);
...
//            }
            ...
            //armPivot.update(targetPivotAngle, 1, 7, 0.69, telemetry);
...
//            }
            ...
            armPivot.update(targetPivotAngle, .9, 15, 0.31, telemetry);
            lift.setSetPoint(liftWantedHeight);
            lift.updateLiftPosition();
        }
        ...
        if (currentState == SuperstructureStates.SPECIMEN_HANG_PREP.ordinal()) {
            if (stateFinished) {
                targetPivotAngle = 80;
                //liftWantedHeight = 0;
                lift.setSetPoint(7.5);
                armPivot.vexIntake.setPower(0);
                armPivot.intakeTilt.setPosition(Constants3Motor.TILT_SERVO_BACK_SPECIMEN_PLACE_PREP);
                armPivot.intakeTiltTwo.setPosition(Constants3Motor.TILT_SERVO_BACK_SPECIMEN_PLACE_PREP);
                armPivot.intakeJawServo.setPosition(Constants3Motor.JAW_SERVO_GRAB_POSITION);
                armPivot.twist.setPosition(Constants3Motor.TWIST_SERVO_BACK_SPECIMEN_PLACING);
                initializeStateVariables();
            }
...
//            }
            ...
            //lift.updateLiftPosition();
            //armPivot.update(targetPivotAngle, 1, 15, 0.7, telemetry);
            armPivot.update(targetPivotAngle, 0.8, 30, 0.15, telemetry);
        }

        if (currentState == SuperstructureStates.SPECIMEN_HANG_CHAMBER_AUTO.ordinal()) {
            if (stateFinished) {
                liftWantedHeight = 0;// change here if problesm
            ...
            lift.updateLiftPosition();
            //armPivot.update(targetPivotAngle, .8, 20, 0.50, telemetry);
        ...
        if (currentState == SuperstructureStates.SPECIMEN_HANG_CHAMBER_TELE.ordinal()) {
            ...
            lift.updateLiftPosition();
            //armPivot.update(targetPivotAngle, 1, 20, 0.75, telemetry);
            armPivot.update(targetPivotAngle, .5, 20, 0.25, telemetry);
        }
        ...
        if (currentState == SuperstructureStates.SAMPLE_COLLECTION_EXTENSTION.ordinal()) {
            if (stateFinished) {
                ...
                armPivot.vexIntake.setPower(0);
                armPivot.intakeJawServo.setPosition(Constants3Motor.JAW_SERVO_GRAB_POSITION);
                armPivot.setIntakeTiltAngle(-15); // get tilt angle down a little before extension
                taskStartTime2 = SystemClock.uptimeMillis();
                initializeStateVariables();
            }
            if (SystemClock.uptimeMillis()-taskStartTime2 > 375) {  // this waits for the tilt to get parallel before extending added 4/7/2025
                if (!sampleCollected) {
                    if (lift.getLiftExtension() > 0.5 && !collectionMode) {
                        armPivot.intakeJawServo.setPosition(Constants3Motor.JAW_SERVO_INTAKE_POSITION);
                        armPivot.vexIntake.setPower(-.91);
                ...
                if (lift.getLiftExtension() > 7.5) { // captures task start time
                    if (!taskReStartTime) {
                        taskStartTime = SystemClock.uptimeMillis();
                        taskReStartTime = true;
                    }

                    if (SystemClock.uptimeMillis() - taskStartTime > 625) {
                        //collectionMode = false;
...
//                    }
            ...
            //armPivot.update(targetPivotAngle, .9, 15, 0.6, telemetry);
        ...
        if (currentState == SuperstructureStates.SPECIMEN_HANG_FRONT_PREP_AUTO.ordinal()) {
            ...
            if (SystemClock.uptimeMillis()-stateStartTime > 500) {
                ...
                armPivot.intakeTiltTwo.setPosition(Constants3Motor.TILT_SERVO_PARALLEL_WITH_PIVOT+0.05);
            }
